  ...
  RTI::FederateInternalError)
{
 //Parameters to be stored-
  ...
  /*
  static NODE theCallBack={NULL,
		    0,
		    0,
		    0,
      		    RTI::AttributeHandleValuePairSet::AttributeHandleValuePairSet(),
		    (*(RTI::FedTimeFactory::makeZero())),
		    RTI::AttributeHandleSet::AttributeHandleSet(),
		    NULL,
		    NULL,
  &theint};
  */
  ...
  //  time=decode(theFedTime);
  //  time = theFedTime;

  RTIfedTime mytime(0);
  mytime+=theFedTime;
  theCallBack->theTime = theFedTime.getTime();

  cout <<"The time,"<<time;
  theCallBack->theTime=&theFedTime;
 
  //  aCallBack->theTime = (*(RTI::FedTimeFactory::makeZero()));
  
  /* if(theCallBack==NULL)
    cout<<endl<<"Allocation failed"<<endl;
  else
    cout<<endl<<"Allocation worked"<<endl;
  */
  //RTI::FedTime&         grantTime;
  
 
  //  theCallBack.theTime=RTI::FedTime::FedTime();
  //theCallBack->theTime=theFedTime;
  /*RTI::Double doubletime;
  RTIfedTime mytime(0);
  mytime+=theFedTime;
  doubletime=mytime.getTime();
  cout<<"The time in Fed Amb: "<<doubletime<<endl;
  mytime=0;
  cout<<"The time in mytime: "<<mytime.getTime()<<endl;
  mytime+=*theCallBack->theTime;
  cout<<"The time in the queue is: "<<mytime.getTime()<<endl;
  ...
  */
  ...
  //enqueue this callback onto the list for this type of callback
  enqueue(timeAdvanceGrant_q,theCallBack);//theCallBack);
  //  RTIfedTime mytime(0);
  mytime +=*timeAdvanceGrant_q->head->theTime;
  cout<<"----------------------------------"<<endl;
  cout<<"Pointer in fed amb is: "<<timeAdvanceGrant_q->head<<endl;
  cout<<"The time in the queue in fed amb is: "<<mytime.getTime()<<endl;
  cout<<"----------------------------------"<<endl;
