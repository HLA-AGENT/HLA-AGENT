				...
				unsigned *Ref)
{
  ...
  RTI::FederateAmbassadorPtr fedAmbRef = (RTI::FederateAmbassadorPtr)Ref;
...
{			   
  ...
  //This should have a general implementation. At present the
  //RTIfedTime class is used as an implementation of RTI::FedTime. It
  //would be better to be able to pass and manipulate RTI::FedTime
  //objects between the RTI and poplog. 
  RTI::FedTime& passTime=(*(RTI::FedTimeFactory::makeZero()));
  RTIfedTime tempTime(theTime);
  passTime=tempTime;
  ...
  try
    {
      ...
      rtiAmb.deleteObjectInstance(instanceId, passTime, NULL);
    }
  catch ( RTI::Exception& e )
    {
      cerr << "FED: ERROR:" << &e << endl;
...
void enableTimeRegulation(unsigned long theFederateTime,
			  unsigned long theLookahead)
{
  ...
  //would be better to be able to pass and manipulate RTI::FedTime
  //objects between the RTI and poplog. 
  //It basically converts unsigned long into correct class
  RTI::FedTime& passFederateTime=(*(RTI::FedTimeFactory::makeZero()));
  RTIfedTime tempFederateTime(theFederateTime);
  passFederateTime=tempFederateTime;

   //This should have a general implementation. At present the
  //RTIfedTime class is used as an implementation of RTI::FedTime. It
  //would be better to be able to pass and manipulate RTI::FedTime
  //objects between the RTI and poplog. 
  RTI::FedTime& passLookahead=(*(RTI::FedTimeFactory::makeZero()));
  RTIfedTime tempLookahead(theLookahead);
  passLookahead=tempLookahead;
  try
    {
      rtiAmb.enableTimeRegulation(passFederateTime, passLookahead);
    }
  catch ( RTI::Exception& e )
    {
      cerr << "FED: ERROR:" << &e << endl;
...
void timeAdvanceRequest(unsigned long theTime)
{
  ...
  try
    {
      rtiAmb.timeAdvanceRequest(passTime);
    }
  catch ( RTI::Exception& e )
    {
      cerr << "FED: ERROR:" << &e << endl;
...
void tick(double minimum, double maximum){
  rtiAmb.tick(minimum,maximum);
}
