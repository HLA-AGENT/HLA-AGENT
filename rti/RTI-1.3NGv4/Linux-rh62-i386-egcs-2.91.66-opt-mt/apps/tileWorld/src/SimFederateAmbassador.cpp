...
using std::endl;
#endif
SimFederateAmbassador    fedAmb;
RTI::Boolean        timeAdvGrant;
...
unsigned *getFederateAmbassadorRef(){
  unsigned *ref = (unsigned *)&fedAmb;
  return ref;
}
  ...
  RTI::FederateInternalError)
{
  ...
  for(RTI::ULong index =0;index<count;++index){
    ...
    if (!(updateAttributes->ahvp[index].value = 
	  (char *) malloc (updateAttributes->ahvp[index].length) ))
      {
	/* no memory left, die */
	exit(1);
      }
  ...
  RTI::FederateInternalError)
{
  ...
  AttributeHandleSet_t *attributeArray= new AttributeHandleSet_t;;
  attributeArray->attributes = new RTI::ULong[count];
  
  //Loop through all the attribute handle pairs
  for(RTI::ULong index =0;index<count;++index){
    //get the handle
    attributeArray->attributes[index] = ahset.getHandle(index);
  } 
  //Store values of this callback 
  theCallBack->objectHandle=theObject;
