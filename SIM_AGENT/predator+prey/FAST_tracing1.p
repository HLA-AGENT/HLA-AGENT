...
;;;    newarray([0 399 0 399], 0) -> stable;
    {% dupnum(0, nagents + 1) %} -> agent_state_accesses;
    {% dupnum(0, world_size**2) %} -> stable;
...
;;;    	end_info(ntimesteps);
	...
	lvars nstate_vars = world_size * world_size,
	      ...
	      n;
	nprintf('State accesses: %p', [^agent_state_accesses]);
	printf('Probability:');
	for n from 0 to nagents do
	    printf(' %p agents %p', [^n ^(agent_state_accesses(n + 1) / possible_accesses)]);
	    endfor;
	printf('\n');
	endfor;
    enddefine;
...
define sim_scheduler_pausing_trace(objects, cycle);
    ...
    fast_for i from 1 to length(stable) do
	stable(i) -> n;
	n fi_+ 1 -> j;
	agent_state_accesses(j) fi_+ 1 -> agent_state_accesses(j);
	0 -> stable(i);
	endfor;
    enddefine;

define sim_scheduler_pausing_trace_slow(objects, cycle);
	...
	fast_for j from ymin to ymax do
	    stable(i, j) -> n;
	    agent_state_accesses(n + 1) fi_+ 1 -> agent_state_accesses(n + 1);
	    0 -> stable(i, j);
	    endfor;
	endfor;
    enddefine;
...
define :method update_stable(agent:predator_agent);
	...
	fast_for y from ya - range to ya + range do
		...
		y mod world_width -> ym;
    		;;; Compute the index corresponding to these coordinates,
    		;;; assuming poparray_by_row is true, i.e. the index
    		;;; is the same as the arrayvector subscript
    		((ym - 1) * world_width) + xm -> s;
		;;;[coords ^xa ^ya ^xm ^ym ^s] =>
		stable(s) fi_+ 1 -> stable(s);
		endif;
	    endfor;
       	endfor;
    enddefine;

define :method update_stable_slow(agent:predator_agent);
...
define :method update_stable_slow(agent:prey_agent);
    lvars xa = rc_picx(agent),
 	  ya = rc_picy(agent),
	  range = max(prey_escape_range(agent), 
		      max(prey_flocking_range(agent), food_sense_range(agent))),
