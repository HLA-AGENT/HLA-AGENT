/*  
    ...
    CHANGES...  
    by: Michael Lees date: 7/03/03 
    
    Desc: Updated graphics
    to now use triangles rather than points. Also added rules to allow
    speed matching using acceleration. Fixed minor bug with movement,
    originally the rule 'update' had,
...
vars WIN_X=1100;       ;;;Size of xgraphic window
vars WIN_Y=800;
vars MAX_ACCEL=1;     ;;; Max accel of boid (values above are clipped to this)
vars MIN_ACCEL=(-1);  ;;; Min accel of boid (values below are clipped)
vars MAX_SPEED = 4;   ;;; Max speed of boid
vars MIN_SPEED=1;     ;;; Min speed of boid
vars SEPERATION=90;   ;;; Distance that boids try to obtain between one another
vars SENSOR_RANGE=180; ;;; How far boids can see
vars MIGRATE=90;      ;;; Angle to head to when on own (leader heads here)
...
define :class boid; is sim_movable_agent, rc_rotatable;
    ...
    slot boid_fieldofview = 140; ;;; Angle the boid can see in deg
...
;;; This determines overall heading/accel from the three votes. It
...
define :ruleset boid_rules;
  ...
  ;;; Resets values for calculating new averages this cycle
    ...
    [POP11 if(printlevel(sim_myself)=3) then
	 ['Heading' ^(fh + heading) ^new_heading]=> endif;]
    [DEL 1]
    [DEL 2]
    [DEL 3]
    [DEL 4]
    ...
    ;;; We have totaled all values from sense data now we average them
    ...
    [DEL 5]
    [DEL 6]
    [POP11['The flock is heading:' ^new_heading]=>]
    [flock_heading ?new_heading]
    [flock_speed ?new_speed]
    [flock_xpos ?new_xpos]
    [flock_ypos ?new_ypos]    
    ...
    [LVARS 
     ...
     ;;; am to the flock
     ;;;[heading = sim_myself.boid_heading - arctan(nx / ny)] 
     [heading = move_away(sim_myself.sim_x, 
		      	  sim_myself.sim_y,x,y,
		      	  fh,			  
		      	  sim_myself.boid_heading)]
     ;;; If I'm very close to someone increase the urgency (urgency
    ...
    ==>
    [LVARS 
     [hdif = sim_degrees_diff(MIGRATE,sim_myself.boid_heading)] 
     [h= sim_myself.boid_heading+(random(10) -5)]
    ...
    [POP11 rc_set_axis(sim_myself, (sim_myself.boid_heading-90), true)]
...
;;;Redefine standard cycle printing stuff 
