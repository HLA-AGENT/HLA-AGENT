/*  
    ...
    Authors: Michael Lees &  Elizabeth Gordon
...
vars WIN_X=400;      ;;; Size of xgraphic window
vars WIN_Y=400;       ;;; (Note to me: For full screen use x=1100 y=800)
...
vars SEPERATION=30;   ;;; Distance that boids try to obtain between one another
vars SENSOR_RANGE=50; ;;;How far boids can see
...
define newboid(x,y,heading)->aboid;
    ...
    ;;; Draws grey circles around boids indicating the sensor range.
    if(SHOW_SENS) then
...
define :method sim_sense_agent(a1:boid, a2:boid, dist);
	...
	;;; Different to dist as it doesn't wrap around world
	lvars distance_from_object =
	      sim_distance_from(a1.sim_x,a1.sim_y,a2.sim_x,a2.sim_y);
	lvars x1,y1,x2,y2;
	;;; Now we determine which distance is closer heading
	;;; directly from a1 to a2 or going in 180 deg to that
	;;; direction and goind round the world as it were
	if((distance_from_object-dist) > 0.01) and  
	    (dist < distance_from_object) then
	    ;;; Then the quickest way round is to go round the world
	    ;;; so we should flip heading by 180 deg NOTE: dist is
	    ;;; calculated using sim_distance_wrap which is different
	    ;;; to sim_distance_from
	    (heading_to_object +180) mod 360 -> heading_to_object;
	    endif;
	...
	if(abs(sim_degrees_diff(a1.boid_heading,heading_to_object)) < 
	   ((a1.boid_fieldofview)/2)) then
	    if(printlevel(a1)>2) then
		['**Agent: '^(sim_name(a1)) ' can see ' ^(sim_name(a2))]=>
		endif;
...
define cross_product(my_x, my_y,
		     flock_x,flock_y, 
		     flock_heading)->d;
    ;;; if d > 0 (my_x,my_y) is on right of line pointing in the
    ;;; direction of flock_heading which the point (flock_x,flock_y)
    ;;; lies on. d<0 then on left, d=0 then on same line.
...
;;; This is redefined to allow agents to see round the edge of the
;;; screen as it were, making the environment continuous. This means
;;; an agent at the very bottom right of the environment will sense
;;; another agent at the top left. ie sim_distance between (0,0) and
;;; (WIN_X,WIN_Y)=0
define sim_distance_wrap(x1,y1,x2,y2) -> dist;
    ...
    if xdiff > WIN_X/2 then
	xdiff - WIN_X->xdiff;
    elseif xdiff <= -WIN_X/2 then 
	xdiff + WIN_X->xdiff;
    endif;

    if ydiff > WIN_Y/2 then
	ydiff - WIN_Y->ydiff;
    elseif ydiff <= -WIN_Y/2 then 
	ydiff + WIN_Y->ydiff;
    endif;

    sqrt((xdiff*xdiff) + (ydiff*ydiff)) -> dist;
enddefine;


;;; Redefined using sim_distance_wrap allowing agents to see round world
define :method sim_distance(a1:boid, a2:boid) -> dist;
    ;;; Compute distance between two agents.    
    lvars x1,y1,x2,y2;
    sim_distance_wrap(a1.sim_x,a1.sim_y,a2.sim_x,a2.sim_y)->dist;
enddefine;

;;; The do action used to update the agents position and heading
define :method move_to(agt:boid, h);
    ;;;Caculate new x and y position from speed and heading
    lvars x = (cos(h) * agt.boid_speed) + agt.sim_x,
     	 y = (sin(h) * agt.boid_speed) + agt.sim_y;    
    ...
    if(DRAW_GRAPHICS) then
	rc_set_axis(agt, (agt.boid_heading-90), true);
    endif;

    ;;;Move boid to new x and y position
    sim_move_to(agt, x, y, true);
enddefine;

;;; The do action used to update the agents speed for a given acceleration
define :method change_accel(agt:boid,a);    
    ;;;Scale accel to within max and min allowed
    if(a>MAX_ACCEL) then MAX_ACCEL->a; endif;
    if(a<MIN_ACCEL) then MIN_ACCEL->a; endif;
    
    ;;;apply new accel to speed
    sim_myself.boid_speed+a->sim_myself.boid_speed;
    
    ;;;Scale new speed to within max and min allowed
    if(sim_myself.boid_speed>MAX_SPEED) then 
	MAX_SPEED->sim_myself.boid_speed;
    elseif(sim_myself.boid_speed<MIN_SPEED) then 
	MIN_SPEED->sim_myself.boid_speed; 
    endif;
