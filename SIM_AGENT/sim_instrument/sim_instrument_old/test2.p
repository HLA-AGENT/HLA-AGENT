...
define :method get_shared_accesses_in(o1:sim_processed_instrument_actor,
    ...
    [% for access in o1.accesslist do
    	   if (access.sim_datum_cycle >= initialtime) and 
		   (access.sim_datum_cycle =< endtime) then
    	       access;
	   endif;
       endfor; 
       for update in o1.updatelist do
    	   if (update.sim_datum_cycle >= initialtime) and 
		   (update.sim_datum_cycle =< endtime) then
    	       update;
	   endif;
       endfor; 
       %] -> actor1list;
    
    [Total access made by agent 1 during ^initialtime and ^endtime is 
     ^(length(actor1list))]=>

    [% for access2 in o2.accesslist do
    	   if (access2.sim_datum_cycle >= initialtime) and
    	   	   (access2.sim_datum_cycle =< endtime) then
	       access2;
	   endif;
       endfor; 
        for update2 in o2.updatelist do
    	   if (update2.sim_datum_cycle >= initialtime) and 
		   (update2.sim_datum_cycle =< endtime) then
    	       update2;
	   endif;
       endfor; 
       %] -> actor2list;

    [Total access made by agent 2 during ^initialtime and ^endtime is 
     ^(length(actor2list))]=>

    for accessin in actor1list do

	for access2in in actor2list do
	    if (accessin.sim_datum_object = access2in.sim_datum_object) 
		    and (accessin.sim_datum_slot = access2in.sim_datum_slot) then	
                access2in::shared->shared;
	    	delete(access2in,actor2list)->actor2list;
	    	true->accessinmatched;		    
	    endif;
    	endfor;

    	if(accessinmatched) then
    	    accessin::shared->shared;
    	    false->accessinmatched;
    	endif;    	      
...
define shared_test(actor1list,actor2list)->shared;
    []->shared;
    lvars accessin, access2in,
	 accessinmatched=false;
    for accessin in actor1list do
	for access2in in actor2list do
	    if (accessin = access2in) then		
      	        access2in::shared->shared;
	     	delete(access2in,actor2list)->actor2list;
	    	true->accessinmatched;		    
	    endif;
...
define run_exp(num_runs, num_cycles);
	...
	count_rollback(list120,networkdelay)->num_rollbacks120;
	count_rollback(list4160,networkdelay)->num_rollbacks4160;
	count_rollback(list81100,networkdelay)->num_rollbacks81100;
	count_rollback(list121140,networkdelay)->num_rollbacks121140;
	count_rollback(list161180,networkdelay)->num_rollbacks161180;


/*	get_shared_accesses_in(hd(agents),hd(tl(agents)),251,270)->list251300;
	get_shared_accesses_in(hd(agents),hd(tl(agents)),301,350)->list301350;
	get_shared_accesses_in(hd(agents),hd(tl(agents)),351,400)->list351400;
	get_shared_accesses_in(hd(agents),hd(tl(agents)),401,450)->list401450;
	get_shared_accesses_in(hd(agents),hd(tl(agents)),451,500)->list451500;
*/
	
	

	[The common accesses between 1 and 20 were 
	 ^((length(list120))/2) rollback patterns ^num_rollbacks120]=>

	[The common accesses between 41 and 60 were 
	 ^((length(list4160))/2) rollback patterns ^num_rollbacks4160]=>
	[The common accesses between 81 and 100 were 
	 ^((length(list81100))/2) rollback patterns ^num_rollbacks81100]=>
	[The common accesses between 121 and 140 were 
	 ^((length(list121140))/2) rollback patterns ^num_rollbacks121140]=>
	[The common accesses between 161 and 180 were 
	 ^((length(list161180))/2) rollback patterns ^num_rollbacks161180]=>

/*	[The common accesses between 251 and 300 were 
	 ^((length(list251300))/2)]=>
	[The common accesses between 301 and 350 were 
	 ^((length(list301350))/2)]=>
	[The common accesses between 351 and 400 were 
	 ^((length(list351400))/2)]=>
	[The common accesses between 401 and 450 were 
	 ^((length(list401450))/2)]=>
	[The common accesses between 451 and 500 were 
	 ^((length(list451500))/2)]=>
*/
