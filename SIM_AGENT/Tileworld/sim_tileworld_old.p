...
define :class tileworld is sim_agent;
     ...
     slot new_hole_prob     = 0.1;
     slot new_tile_prob     = 0.1;
     slot new_obstacle_prob = 0.1;
...
define create_new_objects(env);
	...
	newtile(tile_x,tile_y,gridwidth(env),tile_colour(env),'T'><(num_tiles(env)+1),env)->;
    endif;
	...
	newobstacle(obst_x,obst_y,gridwidth(env),colour(env),env)->;
    endif;
enddefine;
...
define valid_coords(env) -> (valid_x, valid_y);
    ...
    while(member_xy(valid_x, valid_y, objects(env))) do
	random_coords(env) -> (valid_x, valid_y);
    endwhile;
enddefine;
...
define go(cycles /* &optional config, drawgraphics */);
    ...
    sim_scheduler(objects(TW), cycles);
enddefine;
